...
let contains ~pattern s =
  let rec loop i =
    if i < 0 then
      ...
      String.sub s i (String.length pattern) = pattern || loop (i - 1)
  in
...
module SelectionList = struct
  ...
  type 'a model = { list : 'a list; cursor : int; filter : string }
  type msg = [ `Cursor of int | `Filter of string | `Nop ]
    ...
    let l =
      List.mapi
        (fun i x ->
          let a =
            if i = cursor then
              [ style "background-color" "#E0E0E0"; scroll_to_show ]
            else
              []
          in
          div
      ...
      [ elt "input" []
          ~a:
            ...
            ; onkeydown (fun e ->
                  ...
                  | 40 ->
                      msg (`Cursor (min (cursor + 1) (List.length instrs - 1)))
                  | 33 -> msg (`Cursor (max (cursor - 15) 0))
                  | 34 ->
                      msg (`Cursor (min (cursor + 15) (List.length instrs - 1)))
                  | 36 -> msg (`Cursor 0)
...
module Initializable = struct
  type 'a model = Initializing | Ready of 'a
  ...
  let update f model msg =
    ...
    | Ready model, `Internal msg ->
        let m, a = f model msg in
        Vdom.return (Ready m) ~c:[ Vdom.Cmd.map internal a ]
    | _ -> Vdom.return model
  ;;
  ...
  let init f = Vdom.return Initializing ~c:[ Vdom.Cmd.map got f ]
