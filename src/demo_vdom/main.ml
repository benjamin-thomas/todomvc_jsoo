  ...
  let view { n } =
    let rec loop acc i =
      if i = 0 then
        ...
        loop (button (string_of_int i) (`Click i) :: acc) (i - 1)
    in
...
module Pair = struct
  ...
  let app app1 app2 =
    ...
    let view (model1, model2) =
      div
        [ map left (memo app1.view model1); map right (memo app2.view model2) ]
    in
...
module Talk1 = struct
  ...
  let view { x; y } =
    elt "div"
      [ button "Increment" Inc; text (Printf.sprintf "x=%d / y=%d" x y) ]
  ;;
...
module DemoSelection = struct
  ...
  type model = { s : string; select : int SelectionList.model }
  ...
  let view { s; select } =
    elt "div"
      [ text s
      ; SelectionList.view string_of_int
          (fun x -> `Internal x)
          (fun x -> `Select x)
          select
      ]
...
module DemoCheckbox = struct
  ...
  let view { checked } =
    div
      ...
      ; elt "input" []
          ~a:[ str_prop "type" "checkbox"; bool_prop "checked" checked ]
      ; elt "input" []
          ~a:[ str_prop "type" "checkbox"; bool_prop "checked" true ]
      ; elt "input" []
          ~a:
            [ str_prop "type" "checkbox"
...
module Issue18_propagation = struct
  ...
  let view = function
    | Unclicked ->
        div
          ~a:[ onclick (fun _ -> Click "outer"); class_ "outer" ]
          [ div ~a:[ class_ "inner" ] [ text "inside the inner div" ]
          ; div
              ~a:[ class_ "inner"; onclick (fun _ -> Click "inner") ]
              [ text "inner div with own click handler" ]
          ; text "outside the inner div"
          ]
    | Clicked s ->
        div
          ~a:[ onclick (fun _ -> Reset) ]
          [ text (Printf.sprintf "Clicked: %s" s) ]
  ;;
...
let run () =
  ...
  let r app =
    ...
    ; if test_dispose then (
        let button = Document.create_element document "button" in
        Element.append_child button
          (Document.create_text_node document "Dispose Application")
        ; Element.add_event_listener button Event.Click
            (fun _ ->
              Element.remove button
              ; Vdom_blit.dispose app
              ; Element.append_child container
                  (Document.create_text_node document "Disposed"))
            false
