...
(* Application data *)
module Model = struct
  type visibility = Completed | Active | All [@@deriving json]
  ...
  type t =
    { tasks : task list; field : string; uid : int; visibility : visibility }
  [@@deriving json]
  (* to save/restore the state in JSON *)

  let empty = { tasks = []; field = ""; uid = 0; visibility = All }
  ...
  let from_json s = Deriving_Json.from_string [%json: t] s
  let to_json m = Deriving_Json.to_string [%json: t] m
end
  ...
  let update a ((r, f) : rp) =
    ...
    let m =
      ...
      | Add ->
        ...
        in
        { m with uid; field = ""; tasks }
      | Update_field field -> { m with field = Js.to_string field }
      ...
      | Check (id, is_compl) ->
        ...
        { m with tasks = List.map update_task m.tasks }
      | Check_all is_compl ->
        let update_task t = { t with completed = is_compl } in
        { m with tasks = List.map update_task m.tasks }
      | Change_visibility visibility -> { m with visibility }
      | Escape id ->
        let unedit_task t =
          if t.id = id then
...
module View = struct
  ...
  let focus_todo_item id =
    ...
    Js.Opt.case (Dom_html.CoerceTo.input e) (fun e -> ()) (fun e -> e##focus)
  ;;
  ...
  let set_task_input v = task_input_dom##.value := Js.string v
  let focus_task_input () = task_input_dom##focus
          ...
          ~a:
            ...
            ; a_onchange (fun evt ->
                  ...
                  Js.Opt.case tgt
                    (fun () -> true)
                    (fun e ->
                       Controller.update (Update_task (todo.id, e##.value)) (r, f)
                     ; true))
  ...
  let task_list ((r, f) : rp) =
    ...
    Html5.(
      section
        ~a:
          [ a_class [ "main" ]
          ; R.Html5.a_style (React.S.map css_visibility react_tasks)
          ]
        [ Html5.input
            ~a:
              (R.filter_attrib (a_checked ())
                 (React.S.map toggle_input_checked react_tasks)
               :: [ a_input_type `Checkbox
                  ; a_class [ "toggle-all" ]
                  ; a_onclick (fun _ ->
                        let m = React.S.value r in
                        Controller.update
                          (Check_all (not (toggle_input_checked m.Model.tasks)))
                          (r, f)
                      ; focus_task_input ()
                      ; true)
                  ])
  ...
  let visibility_swap m ((r, f) : rp) acc (uri, visibility) =
    ...
    let css =
      if visibility = actual_visibility then
        [ "selected" ]
      else
        []
    in
  ...
  let controls ((r, f) : rp) =
    ...
    let button_hidden tasks =
      let tasks_completed, _ =
        List.partition (fun e -> e.Model.completed) tasks
      in
    ...
    let item_left tasks =
      let _, tasks_left = List.partition (fun e -> e.Model.completed) tasks in
      if List.length tasks_left = 1 then
        " item left"
      else
        " items left"
    in
    ...
    let html =
      footer
        ~a:
          [ a_class [ "footer" ]
          ; R.filter_attrib (a_hidden ())
              (React.S.map footer_hidden react_tasks)
          ]
        [ span
        ...
        ; button
            ~a:
              (R.filter_attrib (a_hidden ())
                 (React.S.map button_hidden react_tasks)
               :: a_button)
            [ txt "Clear completed" ]
        ]
    in
    ...
    Html5.(
      footer
        ...
        ; p
            [ txt "Written by "
            ; a
                ~a:[ a_href "https://stephanelegrand.wordpress.com/" ]
                [ txt "Stéphane Legrand" ]
            ]
        ; p
            [ txt "Various code improvements from "
            ; a
                ~a:[ a_href "https://github.com/Drup" ]
                [ txt "Gabriel Radanne" ]
            ]
        ; p
            [ txt "Based on "
            ; a
                ~a:[ a_href "https://github.com/evancz" ]
                [ txt "Elm implementation by Evan Czaplicki" ]
            ]
        ; p
            [ txt "Part of "
  ...
  let view (r, f) =
    Html5.(
      div
        ...
        [ section
            ~a:[ a_class [ "todoapp" ] ]
            [ task_entry (r, f); task_list (r, f); controls (r, f) ]
        ; info_footer
        ])
  ;;
end
...
let main _ =
  ...
  let parent =
    Js.Opt.get
      (doc##getElementById (Js.string "todomvc"))
      (fun () -> assert false)
  in
